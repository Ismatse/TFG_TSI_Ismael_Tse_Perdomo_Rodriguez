...
class Trainer:
    ...
    def train(self):
        ...
        for epoch in range(self.opt.epochs):
            ...
            if (epoch + 1) % self.opt.checkpoint_interval == 0 or epoch == self.opt.epochs - 1:
                self.save_checkpoint(f"model_epoch_{epoch+1}.pt")
            
            # Print epoch summary
            epoch_time = time.time() - epoch_start_time
            print(f"Epoch {epoch+1}/{self.opt.epochs} | Time: {epoch_time:.2f}s | "
            ...
            for cls in val_report:
                if cls not in ['accuracy', 'macro avg', 'weighted avg']:
                    print(f"{cls}: Precision = {val_report[cls]['precision']:.4f}, "
    ...
    def test(self, checkpoint_path=None):
        ...
        for cls in test_report:
            if cls not in ['accuracy', 'macro avg', 'weighted avg']:
                print(f"{cls}: Precision = {test_report[cls]['precision']:.4f}, "
